...
class InsertProducts extends BaseController
{
	...
	{
		add_action('init', array( $this, 'insert_all') );

	}*/
	function insert_all($product_data)
	{
		//$json = file_get_contents(plugins_url().'/hook-me-up-api/my-product-data.json'); // Get json from sample file
		//$products_data = json_decode($json, true); // Decode it into an array
		echo '<pre>';
		//var_dump($products_data);
		echo '</pre>';
		$this->insert_products($product_data);
	...
	function insert_products ($products)
	{
		...
		{
			array_map(array($this, 'insert_product'), $products); // Run 'insert_product' function from above for each product
		}
	}
	...
	function insert_product ($product_data)
	{
		...
		if ( empty($count) ) {
			...
			$this->insert_product_attributes($count->ID, $product_data['available_attributes'], $product_data['variations']); // Add attributes passing the new post id, attributes & variations
			$this->insert_product_variations($count->ID, $product_data['variations']); // Insert variations passing the new post id & variations
		}
	}
